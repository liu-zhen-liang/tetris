...
{
    /// <summary>挑战模式
    /// 模式介绍：和普通模式类似，但是每一关都会出现一个Boss【Boss是一个比普通方块大的字母形状】总共24关前面17关Boss随机，后面7关为【LZL LOVE YT】七个Boss通关显示【杨婷我爱你--刘真梁】
    /// </summary>
    public class ChallengeMode : ModeBase
    {
        /// <summary>创建普通模式对象并设置初始值
        /// 
        /// </summary>
        ...
        public ChallengeMode(GameBase game, Table table, int seed, int id)
        ...
        public override void MoveToTheEnd(Player play)
        {
            ...
            if (base.PerishRowCount >= 480)
            {
                ...
                //弹出框
                Tool.ShowRemindBox("杨婷我爱你！\r\n\t---刘真梁");
                //调用结束方法
                base.Game.GameOver(this);
            }
            else//生成下一个形状
            {
                //调用玩家对象的生成下一个形状方法【如果是新关卡就生成Boss为下一个，否则就是普通模型为下一个】
                play.NextShape(newCustomsPass);
