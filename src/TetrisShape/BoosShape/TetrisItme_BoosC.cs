...
namespace LZLTetris.TetrisShape.BossShape
{
    ...
    /// 
    /// </summary>
    public class TetrisItme_BossC : TetrisItmeBossBase
    {
        ...
        /// 
        /// </summary>
        private TetrisItme_BossC()
        {
            //设置当前俄罗斯方块的形态数量
            base._formCount = TetrisItme_BossC.ModelList.Length;
            //设置当前形状显示的颜色
            base._showColor = TetrisItme_BossC.FixedShowColor;
            //固定颜色
            base._fixedColor = TetrisItme_BossC.FastenColor;
            //方块颜色是否为固定集合
            base._isFixedColorList = TetrisItme_BossC.IsFixedColours;
        }
        /// <summary>创建该形状对象并赋值
        /// 
        /// </summary>
        ...
        public TetrisItme_BossC(int form, Table containerTable, int upIndex)
            : this()
        {
            //设置输入的形态值
            base._form = form;
            ...
            TableIndex[] ti = TetrisItme_BossC.ModelList[form].ToArray();
            //循环更改值设置显示在容器中的位置【计算相对坐标】【将索引位置加入列行】并存储到对象中
            for (int i = 0; i < ti.Length; i++)
            {
                //更改值
                ti[i].CellIndex += upIndex;
                //判断形状形态是否等于1【因为1的形态产生会空一格】
                if (form == 1)
                    //行索引--
                    ti[i].RowIndex--;
            }
            ...
            //设置缩略图的位置
            base._breviary = TetrisItme_BossC.BreviaryList[form];
        }
        /// <summary>旋转方法
        /// 
        /// </summary>
        ...
        public override bool Spin()
        {
            ...
            {
                //得到形态差
                TableIndex difference = TetrisItme_BossC.ModelList[form][i] - TetrisItme_BossC.ModelList[base._form][i];
                //加上形态差
                ti[i] += difference;
            }
            //判断旋转完毕是否合法
