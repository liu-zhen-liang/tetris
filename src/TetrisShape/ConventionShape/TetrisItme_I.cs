...
namespace LZLTetris.ConventionShape
{
    ...
    /// 
    /// </summary>
    public class TetrisItme_I : TetrisItmeBase
    {
        ...
        /// 
        /// </summary>
        public static Color FixedShowColor = Color.FromArgb(255, 0, 0);
        /// <summary>私有构造函数【用于赋值】
        /// 
        /// </summary>
        private TetrisItme_I()
        {
            //设置当前俄罗斯方块的形态数量
            base._formCount = TetrisItme_I.ModelList.Length;
            //设置当前形状显示的颜色
            base._showColor = TetrisItme_I.FixedShowColor;
        }
        /// <summary>创建该形状对象并赋值
        /// 
        /// </summary>
        ...
        public TetrisItme_I(int form, Table containerTable, int upIndex)
            : this()
        {
            //设置输入的形态值
            base._form = form;
            ...
            TableIndex[] ti = TetrisItme_I.ModelList[form].ToArray();
            ...
            //设置缩略图的位置
            base._breviary = TetrisItme_I.BreviaryList[form];
        }
        ...
        public override bool Spin()
        {
            ...
            if (base._form == 0)//形态1
            {
                ...
                //将第四个索引位置加上：(-2,2)，行减二，列加二
                ti[3] += new TableIndex(-2, 2);
            }
            else//形态2
            {
                //强第一个索引位置加上：(-1,1)，行减一，列加一
                ti[0] += new TableIndex(-1, 1);
                //第二个的索引不变
                //将第三个索引位置加上：(1,-1)，行加一，列减一
                ti[2] += new TableIndex(1, -1);
                //将第四个索引位置加上：(2,-2)，行加二，列减二
                ti[3] += new TableIndex(2, -2);
            }
            //判断旋转完毕是否合法
