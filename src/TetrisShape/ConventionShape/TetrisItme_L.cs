...
namespace LZLTetris.ConventionShape
{
    ...
    /// 
    /// </summary>
    public class TetrisItme_L : TetrisItmeBase
    {
        ...
        /// 
        /// </summary>
        public static Color FixedShowColor = Color.FromArgb(0, 255, 0);
        /// <summary>私有构造函数【用于赋值】
        /// 
        /// </summary>
        private TetrisItme_L()
        {
            //设置当前俄罗斯方块的形态数量
            base._formCount = 4;
            ////设置Guid值
            //base._id = new Guid("FEEEFFF5-45CC-4C6C-8972-824F0BCA488A");
            //设置当前形状显示的颜色
            base._showColor = TetrisItme_L.FixedShowColor;
        }
        /// <summary>创建该形状对象并赋值
        /// 
        /// </summary>
        ...
        public TetrisItme_L(int form, Table containerTable, int upIndex)
            : this()
        {
            //设置输入的形态值
            base._form = form;
            ...
            TableIndex[] ti = TetrisItme_L.ModelList[form].ToArray();
            ...
            //设置缩略图的位置
            base._breviary = TetrisItme_L.BreviaryList[form];
        }
        /// <summary>旋转方法，将当前L字型形状进行旋转
        /// 
        /// </summary>
        ...
        public override bool Spin()
        {
            ...
            if (base._form == 0)//形态1
            {
                //将第一个索引位置加上：(1,1)，行加一，列加一
                ti[0] += new TableIndex(1, 1);
                //将第二个索引不变
                //将第三个索引位置加上：(-1,-1)，行减一，列减一
                ti[2] += new TableIndex(-1, -1);
                //将第四个索引位置加上：(0,-2)，行加零，列减二
                ti[3] += new TableIndex(0, -2);
            }
            else if (base._form == 1)//形态2
            {
                //将第一个索引位置加上：(1,-1)，行加一，列减一
                ti[0] += new TableIndex(1, -1);
                //将第二个索引不变
                //将第三个索引位置加上：(-1,1)，行减一，列加一
                ti[2] += new TableIndex(-1, 1);
                //将第四个索引位置加上：(-2,0)，行减二，列加零
                ti[3] += new TableIndex(-2, 0);
            }
            else if (base._form == 2)//形态3
            {
                //将第一个索引位置加上：(-1,-1)，行减一，列减一
                ti[0] += new TableIndex(-1, -1);
                //将第二个索引不变
                //将第三个索引位置加上：(1,1)，行加一，列加一
                ti[2] += new TableIndex(1, 1);
                //将第四个索引位置加上：(0,2)，行加零，列加二
                ti[3] += new TableIndex(0, 2);
            }
            else//形态4
            {
                //将第一个索引位置加上：(-1,1)，行减一，列加一
                ti[0] += new TableIndex(-1, 1);
                //将第二个索引不变
                //将第三个索引位置加上：(1,-1)，行加一，列减一
                ti[2] += new TableIndex(1, -1);
                //将第四个索引位置加上：(2,0)，行加二，列加零
                ti[3] += new TableIndex(2, 0);
            }
            //判断旋转完毕是否合法
