...
namespace WindowsTableControl
{
    ...
    public class TR : TableControl, IDraw
    {
        ...
        public void ResetTD()
        {
            ...
            {
                //调用重置方法
                this.tds[i].ResetAttribute();
            }
        }
        /// <summary>将指定行的固定单元格元素移入当前单元格
        /// 
        /// </summary>
        /// <param name="tr">要被移动的行对象</param>
        /// <returns>返回true代表移动成功，并且移动的那一行不是空行【如果一行都是空的单元格或者只有活动单元格就是空行】，反之返回false</returns>
        public bool MoveTR(TR tr)
        {
            //创建判断是否为空行的变量，默认为false如果更改一次就赋值为true
            bool Is = false;
            //开始移动
            for (int i = 0; i < tr.tds.Length; i++)
            {
                //得到tag标签
                CellInformation ci = tr.tds[i].Tag;
                //判断是否被选中并且不是活动标签
                if (ci.IsSelect && (!ci.IsActive))
                {
                    //将当前的颜色赋值给当前对象的指定索引位置的单元格
                    this.tds[i].BackgroundColor = tr.tds[i].BackgroundColor;
                    //赋值tag变量
                    this.tds[i].Tag = tr.tds[i].Tag;
                    //将原单元格清除
                    tr.tds[i].ResetAttribute();
                    //赋值为true
                    Is = true;
                }
            }
        ...
        /// 
        /// </summary>
        public void LeftExcursion()
        {
            //创建存储单元格信息对象
            CellInformation oneCInf = this.tds[0].Tag;
            //记录颜色
            Color oneColor = this.tds[0].BackgroundColor;
            //循环进行移动
            for (int i = 1; i < this.tds.Length; i++)
            {
                //得到当前信息对象
                CellInformation cInf = this.tds[i].Tag;
                //判断当前单元格是否被选中，并且不是活动的单元格
                if (cInf.IsSelect && !cInf.IsActive)
                {
                    //移动单元格颜色
                    this.tds[i - 1].BackgroundColor = cInf.ShowColor;
                    //移动单元格信息
                    this.tds[i - 1].Tag = cInf;
                }
            }
            //将第一个移动到最后一个
            this.tds[this.tds.Length - 1].BackgroundColor = oneCInf.ShowColor;
            //强单元格信息移动
            this.tds[this.tds.Length - 1].Tag = oneCInf;
        }
    }
}
