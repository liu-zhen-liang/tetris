...
namespace WindowsTableControl
{
    ...
    public struct TableIndex : IEquatable<TableIndex>, IComparable<TableIndex>
    {
        ...
        /// <summary>两个表格索引位置对象相加方法
        /// 
        /// </summary>
        /// <param name="d1">表格索引对象一</param>
        /// <param name="d2">表格索引对象二</param>
        /// <returns>相加后的值</returns>
        public static TableIndex operator +(TableIndex d1, TableIndex d2)
        {
            //创建存储相加后的值并返回
            return new TableIndex(d1._rowIndex + d2._rowIndex, d1._cellIndex + d2._cellIndex);
        }
        /// <summary>两个表格索引位置对象相减方法
        /// 
        /// </summary>
        /// <param name="d1">表格索引对象一</param>
        /// <param name="d2">表格索引对象二</param>
        /// <returns>相减后的值</returns>
        public static TableIndex operator -(TableIndex d1, TableIndex d2)
        {
            //创建存储相减后的值对象，并返回
            return new TableIndex(d1._rowIndex - d2._rowIndex, d1._cellIndex - d2._cellIndex);
